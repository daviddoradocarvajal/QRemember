            ...
            if (resultSet.next()) {
                if (resultSet.getString("pregunta_seguridad").trim().equals(usuario.getPregunta().trim())) {
                    if (resultSet.getString("respuesta").trim().equals(usuario.getRespuesta().trim())) {
                        ...
                        intent.putExtra("Usuario",tvEmailRecuperar.getText().toString());
                        activity.startActivity(intent);
                        activity.finish();
                        return true;
                    } else {
                        Utils.AlertDialogGenerate(activity,activity.getString(R.string.err),"Respuesta Incorrecta");
                        return false;
                    }
                } else {
                    Utils.AlertDialogGenerate(activity,activity.getString(R.string.err),"Pregunta de seguridad Incorrecta");
                    return false;
                }
            } else {
                Utils.AlertDialogGenerate(activity,activity.getString(R.string.err),"Email incorrecto");
                return false;
            }
        }catch (Exception e){
